...
export async function FeatureHighlights() {
  ...
  return (
    ...
    <section className="trust-section mt-6 md:mt-8 md:pb-12" data-reveal>
      <ul className="trust-bar flex overflow-x-auto scrollbar-none py-4 md:grid md:grid-cols-2 md:gap-y-5 md:overflow-visible md:px-4 md:py-5 lg:grid-cols-4 lg:gap-x-6 lg:px-6 lg:py-4 xl:gap-x-14 xl:px-8">
        {items.map(({ icon: Icon, label }, i) => (
          <li
            key={label}
            className={`flex w-[62%] shrink-0 items-center gap-3 px-6 md:w-auto md:px-2 lg:justify-start lg:pl-6 lg:pr-0 xl:pl-8 ${i > 0 ? "max-md:border-l max-md:border-[var(--color-border-dark)] lg:border-l lg:border-[var(--color-border)]" : ""}`}
          >
            <Icon className="h-10 w-10 shrink-0 md:h-8 md:w-8 lg:h-10 lg:w-10 xl:h-14 xl:w-14" strokeWidth={1} />
